...
def load_csv(csv_path, frame_dir=None):
    ...
    xyvs = {}
    for fid, visi, x, y, l, theta in zip(fids, visis, xs, ys, lens, angles):
    ...
    for match in matches:
        ...
        for clip_name in clip_names:
            ...
            clip_csv_path = osp.join(
                root_dir, match, csv_dirname, "{}.csv".format(clip_name)
            )
    ...
    def __init__(
        ...
        if self._load_train or self._load_train_clip:
            train_outputs = self._gen_seq_list(
                self._train_matches, self._train_num_clip_ratio
            )
        ...
        if self._load_test or self._load_test_clip:
            test_outputs = self._gen_seq_list(
                self._test_matches, self._test_num_clip_ratio
            )
        ...
        for match in matches:
            ...
            for clip_name in clip_names:
                ...
                for i in range(len(ball_xyvs) - 1):
                    xy1, visi1 = (
                        ball_xyvs[i]["center"].xy,
                        ball_xyvs[i]["center"].is_visible,
                    )
                    xy2, visi2 = (
                        ball_xyvs[i + 1]["center"].xy,
                        ball_xyvs[i + 1]["center"].is_visible,
                    )
                    if visi1 and visi2:
