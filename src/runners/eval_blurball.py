...
from utils import mkdir_if_missing, draw_frame, gen_video, Center, BlurEvaluator
from utils.heatmap import gen_line_binary_map
    ...
    for cnt, img_path in enumerate(result_dict.keys()):
        ...
        if vis_frame_dir is not None:
            ...
            for cnt2, img_path2 in enumerate(result_dict.keys()):
                ...
                color_gt = (
                    int(cm_gt(cnt2)[2] * 255),
                    int(cm_gt(cnt2)[1] * 255),
                    int(cm_gt(cnt2)[0] * 255),
                )
                color_pred = (0, 0, 255)
                vis_gt = draw_frame(vis_gt, center=center_gt, color=color_gt, radius=8)
...
class BlurVideosInferenceRunner(BaseRunner):
    def __init__(
        self,
        cfg: DictConfig,
    ...
    def _run_model(self, model=None):
        ...
        evaluator = BlurEvaluator(self._cfg)
        ...
        for key, dataloader_and_gt in self._clip_loaders_and_gts.items():
            ...
            if self._vis_result:
                vis_frame_dir = osp.join(
                    self._output_dir, "{}_{}".format(match, clip_name)
                )
                mkdir_if_missing(vis_frame_dir)
            if self._vis_hm:
                vis_hm_dir = osp.join(
                    self._output_dir, "{}_{}".format(match, clip_name), "hm"
                )
                mkdir_if_missing(vis_hm_dir)
            if self._vis_traj:
                vis_traj_dir = osp.join(self._output_dir, "vis_traj")
                mkdir_if_missing(vis_traj_dir)
                vis_traj_path = osp.join(
                    vis_traj_dir, "{}_{}.png".format(match, clip_name)
                )

            log.info("eval @ match={}, clip={}".format(match, clip_name))

            fp1_im_list, tmp = inference_video(
                detector,
                tracker,
                dataloader,
                self._cfg,
        ...
        evaluator.print_results(
            txt="{} @ dist_threshold={}".format(
                self._cfg["model"]["name"], evaluator.dist_threshold
            ),
