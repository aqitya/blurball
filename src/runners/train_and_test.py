...
class Trainer(BaseRunner):
    def __init__(
        ...
        self._test_after_train_with_best = cfg["runner"]["test"][
            "run_after_train_with_best"
        ]
        ...
        self._inference_video_before_train = cfg["runner"]["inference_video"][
            "run_before_train"
        ]
        self._inference_video_after_train_with_best = cfg["runner"]["inference_video"][
            "run_after_train_with_best"
        ]
        if self._step_inference_video < 1:
            raise ValueError(
                "step_inference_video >= 1, but is set to {}".format(
                    self._step_inference_video
                )
            )
        ...
        log.info(
            "# model params: (trainable) {}, (whole) {}".format(
                count_params(self._model),
        ...
        (
            self._train_loader,
            self._test_loader,
            self._train_clip_loaders_and_gts,
            self._test_clip_loaders_and_gts,
        ) = build_dataloader(cfg)
        self._loss_criteria = build_loss_criteria(cfg)
        self._optimizer, self._scheduler = build_optimizer_and_scheduler(
            cfg, list(self._model.parameters()) + list(self._loss_criteria.parameters())
        )
        ...
        if self._run_inference_video:
            self._vi_runner = VideosInferenceRunner(
                self._cfg,
        ...
        if len(self._find_fp1_epochs) > 0:
            ...
            self._train_vi_runner = VideosInferenceRunner(
                self._cfg,
    ...
    def run(self):
        ...
        for epoch in range(self._max_epoch):
            log.info(
                "(TRAIN) Epoch {}, lr: {}".format(
                    epoch + 1, self._scheduler.get_last_lr()
                )
            )
            ...
            if epoch + 1 in self._find_fp1_epochs:
                ...
                update_fp1_example(
                    epoch + 1, self._model, self._train_vi_runner, self._fp1_fpath
                )
            ...
            if (epoch + 1 > self._start_inference_video) and (
                (epoch + 1) % self._step_inference_video == 0
                or (epoch + 1) == self._max_epoch
            ):
                if self._run_inference_video:
                    ...
                    if is_best:
                        log.info(
                            "best f1 ({}) + acc ({}) @ epoch {}".format(
                                vi_results["f1"], vi_results["accuracy"], epoch + 1
                            )
                        )
            ...
            save_checkpoint(
                ...
                is_best,
                model_path=osp.join(
                    self._output_dir, "checkpoint_ep{}.pth.tar".format(epoch + 1)
                ),
